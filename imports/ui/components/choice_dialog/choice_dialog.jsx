...
export class InputBox extends Component {
	...
	componentDidMount() {
		var self = this;
		$("#choice-dialog.modal").modal();
		$("#choice-dialog.modal").on("hidden.bs.modal", function (e) {
	...
	onClickSubmitButton(e) {
		...
		if(this.props.choiceType === "radio") {
			val = $("#choice-dialog.modal").find("[name='input-select']:checked").val();
		} else {
			val = $("#choice-dialog.modal").find("[name='input-select']").val();
		}
	...
	render() {
		var self = this;
		return (
			<div id="choice-dialog" className="modal" tabIndex="-1" role="dialog">
