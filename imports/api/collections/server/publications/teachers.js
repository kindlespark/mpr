...
Meteor.publish("teacher_list", function() {
	return Teachers.publishJoinedCursors(Teachers.find({}, {sort:{teacher:1}}));
});

Meteor.publish("teachers_empty", function() {
	return Teachers.publishJoinedCursors(Teachers.find({_id:null}, {}));
});

Meteor.publish("teacher_details", function(teacherId) {
	return Teachers.publishJoinedCursors(Teachers.find({_id:teacherId}, {}));
});

Meteor.publish("teacher_list_paged", function(extraOptions) {
	extraOptions.doSkip = true;
	return Teachers.publishJoinedCursors(Teachers.find(databaseUtils.extendFilter({}, extraOptions), databaseUtils.extendOptions({sort:{teacher:1}}, extraOptions)));
});
