...
Meteor.publish("classroom_list", function() {
	return Classrooms.publishJoinedCursors(Classrooms.find({ownerId:this.userId}, {sort:{name:1}}));
});

Meteor.publish("classrooms_empty", function() {
	return Classrooms.publishJoinedCursors(Classrooms.find({_id:null,ownerId:this.userId}, {}));
});

Meteor.publish("classroom_details", function(classroomId) {
	return Classrooms.publishJoinedCursors(Classrooms.find({_id:classroomId,ownerId:this.userId}, {}));
});

Meteor.publish("classroom_list_paged", function(extraOptions) {
	extraOptions.doSkip = true;
	return Classrooms.publishJoinedCursors(Classrooms.find(databaseUtils.extendFilter({ownerId:this.userId}, extraOptions), databaseUtils.extendOptions({sort:{name:1}}, extraOptions)));
});
